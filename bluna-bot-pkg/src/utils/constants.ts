export const BLUNA_CONTRACT_ADDRESS =
  "terra1kc87mu460fwkqte29rquh4hc20m54fxwtsx7gp";

export const LUNA_BLUNA_SWAP_CONTRACT_ADDRESS =
  "terra1jxazgm67et0ce260kvrpfv50acuushpjsz2y0p";

// base64 encoding for `{"swap":{}}`
export const BASE64_EMPTY_SWAP_MESSAGE = "eyJzd2FwIjp7fX0=";

// Default configurations
export const DEFAULT_INTERVAL = 2;
export const DEFAULT_MIN_PERCENTAGE_GAIN = 10;
export const DEFAULT_MAX_PERCENTAGE_LOSS = 1.5;
export const DEFAULT_MAX_SWAP_AMOUNT = 2500;
export const DEFAULT_MIN_SWAP_AMOUNT = 10;
export const DEFAULT_SLIPPAGE_PERCENTAGE = 0.5;
export const DEFAULT_CALLBACK = () => {};
